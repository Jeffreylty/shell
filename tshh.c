...
int builtin_cmd(char **argv) 
{
    if (strcmp(argv[0], "quit")==0){//if quit
        exit(0);
    }
    if (strcmp(argv[0], "fg")==0 || strcmp(argv[0], "bg")==0){//if bg or fg
        do_bgfg(argv);
        return 1;
    }
    if (strcmp(argv[0], "jobs")==0){//if jobs
        listjobs(jobs);
        return 1;
    }
    return 0;//if not built in cmd
}
...
void do_bgfg(char **argv) 
{
    if (argv[1] == NULL) {//if input has no second argument
        printf("%s command requires PID or %%jobid argument\n", argv[0]);
        return;
    }
    
    if (!isdigit(argv[1][0]) && argv[1][0] != '%') {//if input has invlid second argument
    ...
    if (is_job_id) {//if JID
        ...
        if (job == NULL) {//if job does not exist
    ...
    //do bg or fg
    if(strcmp(argv[0], "bg") == 0) {//if bg
        job->state = BG;//set state to bg
        printf("[%d] (%d) %s", job->jid, job->pid, job->cmdline);//print bg info
...
void waitfg(pid_t pid)
{
    ...
    if (job != NULL){//if job exists
        while (job->state == FG){//if job is foreground
            sleep(1);//wait 1 sec
        }
    }
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0){//get the pid
        if (WIFSIGNALED(status)){//if child was terminated by a signal
            deletejob(jobs,pid);
            printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
        }
        if (WIFSTOPPED(status)){//if child is currently stopped
            getjobpid(jobs, pid)->state = ST;
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, WSTOPSIG(status));
        }
        if (WIFEXITED(status)){//if child normally terminated
            deletejob(jobs,pid);
        }
    }
